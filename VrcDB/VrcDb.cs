...
namespace Ripper
{
	public class VrcDb
	{
		...
		SQLiteCommand upsertAndReturnUserId = null;
		SQLiteCommand upsertAndReturnWorldId = null;
		SQLiteCommand upsertAndReturnAvatarId = null;
		...
		public bool Open(string path)
		{
			...
			upsertAndReturnUserId = new SQLiteCommand(
				$"INSERT INTO users(vrc_id, is_legacy, is_removed, name, displayname, trust_level, developer_type)" +
				$"VALUES (@VrcId, @IsLegacy, @IsRemoved, @Name, @DisplayName, @TrustLevel, @DeveloperType)" +
				$"ON CONFLICT(vrc_id) DO UPDATE SET is_removed=@IsRemoved, displayname=@DisplayName, trust_level=@TrustLevel, developer_type=@DeveloperType;" +
				$"SELECT id FROM users WHERE vrc_id=@VrcId;"
				, connection)
			{
				CommandType = CommandType.Text
			};
			upsertAndReturnAvatarId = new SQLiteCommand(
				$"INSERT INTO avatars(vrc_id, is_removed, name, version, author, publicity, created_at, updated_at)" +
				$"VALUES (@VrcId, @IsRemoved, @Name, @Version, @Author, @Publicity, @CreatedAt, @UpdatedAt)" +
				$"ON CONFLICT(vrc_id) DO UPDATE SET is_removed=0, name=@Name, version=@Version, publicity=@Publicity, updated_at=@UpdatedAt;" +
				$"SELECT id FROM avatars WHERE vrc_id=@VrcId;"
				, connection)
			{
				CommandType = CommandType.Text
			};
			upsertAndReturnWorldId = new SQLiteCommand(
				"INSERT INTO worlds(vrc_id, is_legacy, is_removed, name, capacity, version, author, publicity, created_at, updated_at) VALUES" +
				$"(@VrcId, @IsLegacy, @IsRemoved, @Name, @Capacity, @Version, @Author, @Publicity, @CreatedAt, @UpdatedAt)" +
				$"ON CONFLICT(vrc_id) DO UPDATE SET is_removed=@IsRemoved, name=@Name, capacity=@Capacity, version=@Version, publicity=@Publicity, updated_at=@UpdatedAt;" +
				$"SELECT id FROM worlds WHERE vrc_id=@VrcId;"
				, connection)
			{
				CommandType = CommandType.Text
			};
		...
		public DbAvatar GetAvatar(VrcId id)
		{
			String compressedId = id.Guid.ToString("N");
			...
			using (command = new SQLiteCommand(connection))
			{
				command.CommandText = $"SELECT id,is_removed,name,version,author,publicity,created_at,updated_at FROM avatars WHERE vrc_id='{compressedId}';";
		...
		public DbWorld GetWorld(VrcId id)
		{
			String compressedId = id.Guid.ToString("N");
			bool isLegacy = id.IdType == VrcId.VrcIdType.LegacyWorld;
			...
			using (command = new SQLiteCommand(connection))
			{
				command.CommandText = $"SELECT id,is_removed,name,capacity,version,author,publicity,created_at,updated_at FROM worlds WHERE vrc_id='{compressedId}';";
